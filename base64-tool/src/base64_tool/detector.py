...
def _score_rot13(data: str) -> float:
    """Score how likely a string is ROT13-encoded English."""
    # Use try_decode to safely attempt decoding without circular imports
    decoded = try_decode(data, EncodingFormat.ROT13)
    if decoded is None:
        return 0.0
    try:
    ...
    # If the decoded text is identical to the input, it's not ROT13 encoded
    if decoded_text == data:
        return 0.0
        
    # Base score for successfully decoding into valid text that changes from input
    score = 0.5
    
    # Boost score if common English words/patterns appear in the decoded text
    common_words = {"the", "and", "ing", "ion", "you", "for", "that", "with"}
    words = set(decoded_text.lower().split())
    if words.intersection(common_words):
        score += 0.4

    return min(score, 1.0)

def _score_ascii85(data: str) -> float:
    stripped = data.strip()
    if len(stripped) < MIN_INPUT_LENGTH:
        return 0.0

    # Handle Adobe wrappers for character checking
    check_data = stripped
    has_wrapper = False
    if check_data.startswith("<~") and check_data.endswith("~>"):
        has_wrapper = True
        check_data = check_data[2:-2]

    # Verify characters are within ASCII85 charset
    if not all(c in ASCII85_CHARSET for c in check_data):
        return 0.0

    score = 0.35
    if has_wrapper:
        score += 0.25  # High confidence boost for Adobe PDF standard wrapper

    decoded = try_decode(stripped, EncodingFormat.ASCII85)
    if decoded is None:
        return 0.0
...
_SCORERS: dict[EncodingFormat,
               Callable[[str],
                        float]] = {
                            ...
                            EncodingFormat.ROT13: _score_rot13,
                            EncodingFormat.ASCII85: _score_ascii85,
                        }
