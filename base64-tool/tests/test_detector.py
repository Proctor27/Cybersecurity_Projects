...
from base64_tool.detector import detect_best, detect_encoding, EncodingFormat
...
class TestDetectRot13:
    def test_detect_rot13_valid(self) -> None:
        """Checks that a ROT13-encoded string with common English words is detected correctly."""
        sample = "Uryyb jbeyq gur naq"
        result = detect_best(sample)
        assert result is not None
        assert result.format == EncodingFormat.ROT13
        assert result.confidence > 0.5

    def test_rot13_unchanged_text_ignored(self) -> None:
        """Checks that plain text or text that doesn't change under ROT13 isn't falsely scored."""
        # Numbers or symbols that don't shift
        sample = "1234567890"
        result = detect_best(sample)
        # Should either be None or not detected as ROT13 with high confidence
