...
class TestSingleLayer:
    ...
    def test_peel_base32(self) -> None:
        ...
        assert result.success is True
        assert len(result.layers) >= 1
    def test_peel_rot13(self) -> None:
        """
        Checks that a single ROT13 layer is successfully detected and peeled
        """
        # If encode supports ROT13:
        encoded = encode(b"Hello World the and", EncodingFormat.ROT13)
        result = peel(encoded, max_depth=1)
        assert result.success is True
        assert len(result.layers) == 1
        assert result.layers[0].format == EncodingFormat.ROT13
        assert b"Hello World" in result.final_output
