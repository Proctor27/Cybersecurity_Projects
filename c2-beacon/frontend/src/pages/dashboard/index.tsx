...
function BeaconRow({
  ...
  return (
    ...
    >
      ...
      <td>{formatUptime(beacon.first_seen)}</td>
...
export function Component(): React.ReactElement {
  ...
  const fetchOperatorCount = async () => {
    try {
      const res = await fetch('/api/operators/count');
      if (res.ok) {
        const data = await res.json();
        setOperatorCount(data.count);
      }
    } catch (err) {
      console.error('Failed to fetch operator count', err);
    }
  };

  fetchOperatorCount();
  const interval = setInterval(fetchOperatorCount, 5000); // Refresh every 5s
  return () => clearInterval(interval);
}, []);
      ...
      <div className={styles.container}>
        <div className={styles.header}>
          ...
          <span className={styles.count}>
            ...
            <span>{operatorCount} {operatorCount === 1 ? 'operator' : 'operators'} connected</span>
        </div>
