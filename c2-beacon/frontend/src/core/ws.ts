...
const AUTH_KEY = import.meta.env.VITE_AUTH_KEY || 'operator-default-key'
  ...
  useEffect(() => {
    ...
    function connect(): void {
      ...
      ws.onopen = () => {
        attempt = 0
        setConnected(true)
        // Send initial authentication payload to operator WebSocket
      ws.send(
       JSON.stringify({
       type: 'auth',
       key: "c2-operator-default-secret-auth-key",
       role: "admin",
    })
  )
}
      ...
      ws.onclose = () => {
        setConnected(false)
          ...
          // Prevent infinite reconnect loops if authentication fails
      if (event.code === 4001) {
    toast.error('Operator authentication failed: invalid key')
    return
  }
